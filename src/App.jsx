...
function Spinner({message="Memuat data..."}){
  return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",flexDirection:"column",gap:12,background:"#f5f6f8"}}>
    <div style={{width:48,height:48,border:"4px solid #e5e7eb",borderTop:"4px solid #185FA5",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
    <div style={{fontSize:14,color:"#555",fontWeight:500}}>{message}</div>
    <div style={{fontSize:12,color:"#aaa"}}>Mohon tunggu sebentar...</div>
    <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
  </div>);
}

function ErrorScreen({message,onRetry}){
  return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",flexDirection:"column",gap:16,background:"#f5f6f8",padding:24}}>
    <div style={{fontSize:56}}>😕</div>
    <div style={{fontSize:18,fontWeight:700,color:"#333",textAlign:"center"}}>Oops! Ada Masalah</div>
    <div style={{fontSize:14,color:"#666",textAlign:"center",maxWidth:380,lineHeight:1.6}}>{message}</div>
    <div style={{background:"#fff",border:"1px solid #eee",borderRadius:12,padding:"1rem 1.5rem",maxWidth:380,width:"100%"}}>
      <div style={{fontSize:12,fontWeight:600,color:"#888",marginBottom:8,textTransform:"uppercase",letterSpacing:.5}}>Yang bisa dicoba:</div>
      {["Periksa koneksi internet Anda","Coba refresh halaman (tekan F5)","Tunggu beberapa menit lalu coba lagi","Jika masalah berlanjut, hubungi admin"].map((t,i)=>(
        <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:6,fontSize:13,color:"#555"}}>
          <span style={{color:"#1D9E75",flexShrink:0,marginTop:1}}>✓</span>{t}
        </div>
      ))}
    </div>
    {onRetry&&(
      <button onClick={onRetry} style={{padding:"10px 28px",background:"#185FA5",color:"#fff",border:"none",borderRadius:8,fontSize:14,cursor:"pointer",fontWeight:600}}>
        🔄 Coba Lagi
      </button>
    )}
  </div>);
...
function OfflineBar(){
  ...
  return(<div style={{position:"fixed",top:0,left:0,right:0,zIndex:9999,background:"#D85A30",color:"#fff",textAlign:"center",padding:"10px",fontSize:13,fontWeight:500}}>
...
export default function App(){
  ...
  // Daily Process menambahkan project baru langsung ke tabel "projects" (lihat DailyProcess.jsx).
  // Event ini memberi tahu App.jsx untuk reload supaya project baru itu juga muncul
  // di Dashboard / Projects / Gantt / Analytics tanpa perlu refresh manual.
  useEffect(()=>{
    const handler=()=>{if(session) loadProjects();};
    window.addEventListener("mde:projects-changed",handler);
    return()=>window.removeEventListener("mde:projects-changed",handler);
  },[session]);
  ...
  const loadProjects=async()=>{
    ...
    try{
      ...
      if(!navigator.onLine){
        setLoadError("Tidak ada koneksi internet. Pastikan perangkat Anda terhubung ke internet lalu coba lagi.");
      } else {
        setLoadError("Gagal memuat data dari server. Server mungkin sedang dalam pemeliharaan. Coba lagi dalam beberapa menit.");
      }
    }
    setLoading(false);
  };
  ...
  if(loading) return <Spinner message="Memuat data proyek..."/>;
  if(loadError) return <ErrorScreen message={loadError} onRetry={loadProjects}/>;
